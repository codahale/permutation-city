...
#define AES_ROUND(s0, s1, s2, s3, off) \
	MOVOU ·rc+off(SB), X4; \
	AESENC X4, s0; \
	MOVOU ·rc+off+16(SB), X4; \
	AESENC X4, s1; \
	MOVOU ·rc+off+32(SB), X4; \
	AESENC X4, s2; \
	MOVOU ·rc+off+48(SB), X4; \
	AESENC X4, s3
...
#define MIX512(s0, s1, s2, s3) \
	...
	// Round 0
	AES_ROUND(X0, X1, X2, X3, 0)
	AES_ROUND(X0, X1, X2, X3, 64)
	MIX512(X0, X1, X2, X3)

	// Round 1
	AES_ROUND(X0, X1, X2, X3, 128)
	AES_ROUND(X0, X1, X2, X3, 192)
	MIX512(X0, X1, X2, X3)

	// Round 2
	AES_ROUND(X0, X1, X2, X3, 256)
	AES_ROUND(X0, X1, X2, X3, 320)
	MIX512(X0, X1, X2, X3)
