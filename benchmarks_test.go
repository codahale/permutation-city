...
func BenchmarkKeccakP1600x4(b *testing.B) {
	var stateA, stateB, stateC, stateD [200]byte
	b.SetBytes(int64(len(stateA) + len(stateB) + len(stateC) + len(stateD)))
	b.ReportAllocs()
	for b.Loop() {
		keccak.P1600x4(&stateA, &stateB, &stateC, &stateD)
	}
}
